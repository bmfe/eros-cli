...
var print = require('../../../utils/print'),
    getIPAdress = require('./util').getIPAdress,
...
gulp.task('socket', (done) => {
    ...
    if(hotRefresh === 'true') {
        ...
        wss = new WebSocketServer({port: socketServerPort}) 
        wss.on('connection', (ws) => {
            logger.success('Your device had been connected to the socket server.')
            wsInstance = ws
            ws.isAlive = true;
            ws.on('pong', () => {
              this.isAlive = true
    ...
    if(hotRefresh === 'true') logger.log('dev socket started in port : ' + socketServerPort.toString().green)
...
gulp.task('hack.config.native', (done) => {
    ...
    getIPAdress()
    jsonfile.writeFileSync(path.resolve(process.cwd(), 'dist/eros.native.json'), nativeConfig);
    done()
})
...
gulp.task('dev', (callback) => {
    gulpSequence('clean', ['build-assets', 'build-iconfont', 'build-mock', 'build-js'], ['hack.config.native', "compatible-weex-debug", 'start-mock', 'watch', 'socket'])(callback)
});
...
gulp.task('weex-eros:ios', (callback) => {
    gulpSequence('clean', 'weex-eros-ios', 'weex:js')(callback)
});
gulp.task('weex-eros:android', (callback) => {
    gulpSequence('clean', 'weex-eros-android', 'weex:js')(callback)
});
gulp.task('weex-eros:all', (callback) => {
    gulpSequence('clean', 'weex-eros-all', 'weex:js')(callback)
});
