...
gulp.task('weex:js', ['build-js'], (done) => {
    ...
    return merge(tasks).pipe(logger.time('build-weex-js')).on('end', (done) => {
        ...
        shell.mv('-f', _iconpath, _tmppath);
        shell.mv('-f', _assetspath, _tmppath);
...
gulp.task('weex', gulpSequence(
    'clean', ['build-assets', 'build-iconfont'], 'weex:js'
));
...
function buildIconfont(done, callback){
...
function buildAssets(done, callback) {
    gulp.src('src/assets/**/*')
